...
static ssize_t proximity_type_show(struct device *dev,
	struct device_attribute *attr, char *buf)
{
	return snprintf(buf, PAGE_SIZE, "%u\n", 8);
}

static ssize_t proximity_zero_show(struct device *dev,
	struct device_attribute *attr, char *buf)
{
	return snprintf(buf, PAGE_SIZE, "%u\n", 0);
}

static DEVICE_ATTR(type, S_IRUGO, proximity_type_show, NULL);
static DEVICE_ATTR(version, S_IRUGO, proximity_zero_show, NULL);
static DEVICE_ATTR(max_range, S_IRUGO, proximity_type_show, NULL);
static DEVICE_ATTR(resolution, S_IRUGO, proximity_type_show, NULL);
static DEVICE_ATTR(sensor_power, S_IRUGO, proximity_zero_show, NULL);
static DEVICE_ATTR(min_delay, S_IRUGO, proximity_zero_show, NULL);
static DEVICE_ATTR(max_delay, S_IRUGO, proximity_zero_show, NULL);
static DEVICE_ATTR(flags, S_IRUGO, proximity_zero_show, NULL);
static DEVICE_ATTR(fifo_reserved_event_count, S_IRUGO, proximity_zero_show, NULL);
static DEVICE_ATTR(fifo_max_event_count, S_IRUGO, proximity_zero_show, NULL);
